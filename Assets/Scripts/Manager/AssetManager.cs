...
public class AssetManager : Manager
{
    ...
    {
        m_BaseDownloadingURL = PathConst.GetRelativePath();
    ...
    {
        //读取最新的更新文件
        Debug.Log("沙盒路径:" + PathConst.DataPath + PathConst.VersionName);
        string text = File.ReadAllText(PathConst.DataPath + PathConst.VersionName);
    ...
    private bool IsSingleFile(string path)
    {
        ...
        for (int i = 0; i < PathConst.aFolders.Length; ++i)
        {
            string f = PathConst.aFolders[i];
            if (path.Contains(f))
            {
                return false;
            }
        ...
        for (int i = 0; i < PathConst.fFolders.Length; ++i)
        {
            string f = PathConst.fFolders[i];
            if (path.Contains(f))
            {
                return false;
            }
    ...
    {
        if (!abname.EndsWith(PathConst.ExtName))
        {
            abname += PathConst.ExtName;
        }
        AssetBundleInfo bundle = null;
        if (!m_LoadedAssetBundles.ContainsKey(abname))
        {
            byte[] stream = null;
            string uri = PathConst.DataPath + abname;
    ...
    {
        if (abName.Equals(PathConst.AssetDir))
        {
            return abName;
        }
        abName = abName.ToLower();
